...
//		Iterator 인터페이스 : HashSet이나 TreeSet과 같이 입력하는 순서와 저장되는 순서가 달라서 get(), set() 메소드를 사용할 수 없는 객체를 분리한다. 
//		HashSet이나 TreeSet 객체에 iterator() 메소드를 실행하면 데이터 단위로 분리된다.
		Iterator<String> iterator = hset.iterator(); //두부를 한모한모 나누는 행위
...
//			next()메소드는 Iterator 인터페이스 객체의 다음 데이터를 얻어온다.
		...
		hset.remove("홍길동"); //여기 이후로 "홍길동"이 지워짐.
		System.out.println(hset.size() + " : " + hset);
		hset.clear(); //여기 이후로 모든 저장소가 지워짐.
		System.out.println(hset.size() + " : " + hset);
		System.out.println("===============================");
	}
