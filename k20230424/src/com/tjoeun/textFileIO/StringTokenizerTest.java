...
//		split() 메소드는 구분자를 2개 이상 지정할 수 없으므로 공백과 탭이 섞여있는 데이터에 구분자를 공백만 지정하면 탭으로 구분된 데이터는 분리할 수 없다.
		str = "사과 배 복숭아	밤 대추";
...
//		hasMoreTokens() 메소드는 StringTokenizer 클래스 객체에 다음에 읽을 token이 있으면 true, 없으면 false를 리턴한다.
		while(st1.hasMoreTokens()) { //다음에 읽을 데이터(token)가 있는동안 반복한다.
...
//		StringTokenizer 클래스의 생성자의 2번째 인수로 구분자를 지정할 수 있다.
...
//		StringTokenizer 클래스의 생성자의 2번째 인수로 구분자를 '여러개' 지정할 수 있다.
...
//		StringTokenizer 클래스의 생성자의 3번째 인수로 구분자를 token에 포함시키는 여부를 지정할 수 있다.
//		생략시 false가 기본값으로 구분자를 token에 포함시키지 않지만, true를 쓰면 token을 구분자에 포함시킨다.
		StringTokenizer st4 = new StringTokenizer(str4, "=,",true); //(str4, "=,", true);는 구분자도 token으로 구분한다.
